  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        } else if (data.fileError && data.fileError.includes('EROFS')) {
          setMessage(
            '📱 Помилка збереження даних на сервері.\n\n' +
              'Це означає що база даних не налаштована для продакшну.\n' +
              'Будь ласка, зверніться до адміністратора сайту.\n\n' +
              'Технічні деталі: Файлова система доступна тільки для читання (Vercel/serverless)',
          );
        } else {
          setMessage(
            `❌ Помилка при надсиланні форми: ${data.error || 'Невідома помилка сервера'}`,
          );
        }
      }
    } catch (error) {
