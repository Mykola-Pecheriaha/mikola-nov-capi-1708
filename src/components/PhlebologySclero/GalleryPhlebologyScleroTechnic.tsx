...
export type GalleryPhlebologyScleroTechnicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї техніки склеротерапії
const technicImages: GalleryPhlebologyScleroTechnicImage[] = [
  {
    src: "/images/varicose/sclero/scleroter9.jpg",
    alt: "Техніка склеротерапії 1",
    width: 800,
    height: 600,
    title: "Підготовка до процедури"
  },
  {
    src: "/images/varicose/sclero/scleroter10.jpg",
    alt: "Техніка склеротерапії 2",
    width: 800,
    height: 600,
    title: "Введення препарату"
  },
  {
    src: "/images/varicose/sclero/scleroter14.jpg",
    alt: "Техніка склеротерапії 3",
    width: 800,
    height: 600,
    title: "Контроль під УЗД"
  },
  {
    src: "/images/varicose/sclero/scleroter16.jpg",
    alt: "Техніка склеротерапії 4",
    width: 800,
    height: 600,
    title: "Компресія після ін'єкції"
  }
];

interface GalleryPhlebologyScleroTechnicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhlebologyScleroTechnic({ className = "", style }: GalleryPhlebologyScleroTechnicProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(technicImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % technicImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + technicImages.length) % technicImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!technicImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = technicImages[currentIndex];
