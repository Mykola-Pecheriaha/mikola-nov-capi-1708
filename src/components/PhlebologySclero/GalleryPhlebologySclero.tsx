...
export type GalleryPhlebologyScleroImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї склеротерапії
const scleroImages: GalleryPhlebologyScleroImage[] = [
  {
    src: "/images/varicose/sclero/scleroter8.jpg",
    alt: "Склеротерапія 1",
    width: 800,
    height: 600,
    title: "Механізм дії склеротерапії"
  },
  {
    src: "/images/varicose/sclero/scleroter4.jpg",
    alt: "Склеротерапія 2",
    width: 800,
    height: 600,
    title: "Механізм дії склеротерапії 2"
  },
  {
    src: "/images/varicose/sclero/scleroter1.jpg",
    alt: "Склеротерапія 3",
    width: 800,
    height: 600,
    title: "Методика проведення склеротерапії 3"
  },
  {
    src: "/images/varicose/sclero/scleroter2.jpg",
    alt: "Склеротерапія 4",
    width: 800,
    height: 600,
    title: "Схема дії склеротерапії"
  }
];

interface GalleryPhlebologyScleroProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhlebologySclero({ className = "", style }: GalleryPhlebologyScleroProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(scleroImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % scleroImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + scleroImages.length) % scleroImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!scleroImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = scleroImages[currentIndex];
