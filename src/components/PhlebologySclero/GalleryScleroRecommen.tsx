...
export type GalleryScleroRecommenImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї рекомендацій
const recommenImages: GalleryScleroRecommenImage[] = [
  {
    src: "/images/varicose/sclero/scleroter17.jpg",
    alt: "Рекомендації після склеротерапії 1",
    width: 800,
    height: 600,
    title: "Еластичний бинт"
  },
  {
    src: "/images/varicose/sclero/scleroter20.jpg",
    alt: "Рекомендації після склеротерапії 2",
    width: 800,
    height: 600,
    title: "Активність після процедури"
  },
  {
    src: "/images/varicose/sclero/scleroter19.jpg",
    alt: "Рекомендації після склеротерапії 3",
    width: 800,
    height: 600,
    title: "Уникати навантажень"
  },
  {
    src: "/images/varicose/sclero/scleroter18.jpg",
    alt: "Рекомендації після склеротерапії 4",
    width: 800,
    height: 600,
...
interface GalleryScleroRecommenProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryScleroRecommen({ className = "", style }: GalleryScleroRecommenProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(recommenImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % recommenImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + recommenImages.length) % recommenImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!recommenImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = recommenImages[currentIndex];
