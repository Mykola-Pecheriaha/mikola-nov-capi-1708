...
const menu = [
  ...
  { label: 'Видалення або заміна імплантів', href: '/services/removal-replacement-implants' },
  { label: 'Підтяжка грудей', href: '/services/plastic-lift' },
  {
    label: 'Гінекомастія',
    href: '/services/plastic-gynecomastia',
  },
];
export default function SidebarBreastAugment() {
        ...
        <ul className="space-y-3 text-base font-medium">
          {menu
            .filter(
              (item) =>
                !(
                  item.href === '/services/breast-augmentation' &&
                  pathname.startsWith('/services/breast-augmentation')
                ),
            )
            .map((item) => (
              <li key={item.href}>
