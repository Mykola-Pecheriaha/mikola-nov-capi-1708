...
Наше завдання — максимально підтримати вас у цьому періоді, забезпечивши комфорт, безпеку та впевненість на кожному етапі відновлення.`,
  ...
  // Визначаємо класи для кольору тексту
  ...
  return (
    <div
      className="w-full py-16 relative bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800"
      style={backgroundColor ? { backgroundColor } : undefined}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        {/* Заголовок */}
        <h2
          className={`text-center text-2xl md:text-3xl lg:text-4xl font-bold mb-12 text-blue-900 dark:text-gray-100`}
        >
        ...
        <div className="w-full mt-8 space-y-6">
          {consultationSections.map((section) => (
            <div
              key={section.id}
              className="rounded-xl overflow-hidden shadow-lg bg-white dark:bg-gray-800 border border-blue-100 dark:border-gray-700 transition-all duration-300 hover:shadow-xl hover:border-blue-300 dark:hover:border-gray-600"
            >
              {/* Заголовок акордеону */}
              <div
                className="flex justify-between items-center p-4 sm:p-6 bg-white dark:bg-gray-800 cursor-pointer transition-colors duration-300 hover:bg-blue-50 dark:hover:bg-gray-700"
                onClick={() => toggleSection(section.id)}
              >
                <div className="flex items-center gap-3">
                  <span className="text-2xl sm:text-3xl">{section.icon}</span>
                  <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold text-blue-900 dark:text-gray-100">
                    {section.title}
                  </h3>
                </div>
                <span
                  className={`text-2xl sm:text-3xl text-blue-500 dark:text-blue-400 transition-transform duration-300 ${
              ...
              {openSection === section.id && (
                <div className="flex flex-col lg:flex-row p-4 sm:p-6 lg:p-8 bg-blue-50/30 dark:bg-gray-700/30 border-t border-blue-100 dark:border-gray-600 gap-6 lg:gap-8">
                  {/* Текстова колонка */}
