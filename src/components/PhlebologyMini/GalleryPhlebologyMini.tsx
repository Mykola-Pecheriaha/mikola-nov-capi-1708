...
export type GalleryPhlebologyMiniImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const miniImages: GalleryPhlebologyMiniImage[] = [
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy.jpg",
    alt: "Мініфлебектомія 1",
    width: 800,
    height: 600,
    title: "Результат 1"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy1.jpg",
    alt: "Мініфлебектомія 2",
    width: 800,
    height: 600,
    title: "Результат 2"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy2.jpg",
    alt: "Мініфлебектомія 3",
    width: 800,
    height: 600,
    title: "Результат 3"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy3.jpg",
    alt: "Мініфлебектомія 4",
    width: 800,
    height: 600,
    title: "Результат 4"
  }
...
interface GalleryPhlebologyMiniProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhlebologyMini({ className = "", style }: GalleryPhlebologyMiniProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(miniImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % miniImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + miniImages.length) % miniImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!miniImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = miniImages[currentIndex];
