...
export default function SidebarPhleboMini() {
    ...
    <aside className="hidden md:block md:w-1/5 flex-none mt-10">
       <div className="sticky top-24 max-h-[calc(100vh-7rem)] overflow-y-auto bg-white rounded-xl shadow-md p-4 border border-blue-100">
        <ul className="space-y-3 text-base font-medium">
          {menu
            .filter(item => !(item.href === "/services/phlebology-miniphlebectomy" && pathname.startsWith("/services/phlebology-miniphlebectomy")))
            .map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
