...
export type GalleryMiniAdvantImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const miniAdvantImages: GalleryMiniAdvantImage[] = [
  {
    src: "/images/varicose/miniphlebectomy/miniadvant1.jpg",
    alt: "Переваги мініфлебектомії 1",
    width: 800,
    height: 600,
    title: "Малоінвазивність"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniadvant2.jpg",
    alt: "Переваги мініфлебектомії 2",
    width: 800,
    height: 600,
    title: "Маленький прокол"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniadvant3.jpg",
    alt: "Переваги мініфлебектомії 3",
    width: 800,
    height: 600,
    title: "Сучасні інструменти"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniadvant5.jpg",
    alt: "Переваги мініфлебектомії 4",
    width: 800,
    height: 600,
...
interface GalleryMiniAdvantProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryMiniAdvant({ className = "", style }: GalleryMiniAdvantProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(miniAdvantImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % miniAdvantImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + miniAdvantImages.length) % miniAdvantImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!miniAdvantImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = miniAdvantImages[currentIndex];
