...
export type GalleryMiniStageImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const miniStageImages: GalleryMiniStageImage[] = [
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy4.jpg",
    alt: "Мініфлебектомія етап 1",
    width: 800,
    height: 600,
    title: "Місцеве знеболювання"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy5.jpg",
    alt: "Мініфлебектомія етап 2",
    width: 800,
    height: 600,
    title: "Проколи шкіри"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy7.jpg",
    alt: "Мініфлебектомія етап 3",
    width: 800,
    height: 600,
    title: "Висічення вен"
  },
  {
    src: "/images/varicose/miniphlebectomy/miniphlebectomy8.jpg",
    alt: "Мініфлебектомія етап 4",
    width: 800,
    height: 600,
    title: "Накладання шва"
  }
];

interface GalleryMiniStageProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryMiniStage({ className = "", style }: GalleryMiniStageProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(miniStageImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % miniStageImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + miniStageImages.length) % miniStageImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!miniStageImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = miniStageImages[currentIndex];
