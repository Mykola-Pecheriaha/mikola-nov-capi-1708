...
export type GalleryDupuytrenClinicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для клінічної галереї Dupuytren
const clinicImages: GalleryDupuytrenClinicImage[] = [
  {
    src: "/images/hand/clinicdupuytren1.jpg",
    alt: "Клінічний прояв 1",
    width: 800,
    height: 600,
    title: "Клінічний прояв 1"
  },
  {
    src: "/images/hand/clinicdupuytren3.jpg",
    alt: "Клінічний прояв 2",
    width: 800,
    height: 600,
    title: "Клінічний прояв 2"
  },
  {
    src: "/images/hand/clinicdupuytren2.jpg",
    alt: "Клінічний прояв 3",
    width: 800,
    height: 600,
    title: "Клінічний прояв 3"
  },
  {
    src: "/images/hand/clinicdupuytren4.jpg",
    alt: "Клінічний прояв 4",
    width: 800,
    height: 600,
    title: "Клінічний прояв 4"
  },
];

interface GalleryDupuytrenClinicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryDupuytrenClinic({ className = "", style }: GalleryDupuytrenClinicProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(clinicImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % clinicImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + clinicImages.length) % clinicImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!clinicImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = clinicImages[currentIndex]
...
export default GalleryDupuytrenClinic
