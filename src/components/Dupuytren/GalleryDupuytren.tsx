...
export type GalleryDupuytrenImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї Dupuytren
const dupuytrenImages: GalleryDupuytrenImage[] = [
  {
    src: "/images/hand/staddupuytren1.jpg",
    alt: "Дюпюїтрена 1",
    width: 800,
    height: 600,
    title: "Стадія 1"
  ...
  {
    src: "/images/hand/staddupuytren2.jpg",
    alt: "Дюпюїтрена 2",
    width: 800,
    height: 600,
    title: "Стадія 2"
  ...
  {
    src: "/images/hand/staddupuytren3.jpg",
    alt: "Дюпюїтрена 3",
    width: 800,
    height: 600,
    title: "Стадія 3"
  ...
  {
    src: "/images/hand/staddupuytren4.jpg",
    alt: "Дюпюїтрена 4",
    width: 800,
    height: 600,
    title: "Стадія 4"
...
interface GalleryDupuytrenProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryDupuytren({ className = "", style }: GalleryDupuytrenProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(dupuytrenImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % dupuytrenImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + dupuytrenImages.length) % dupuytrenImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!dupuytrenImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = dupuytrenImages[currentIndex]
...
export default GalleryDupuytren
