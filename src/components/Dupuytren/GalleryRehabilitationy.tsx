...
export function GalleryRehabilitationy({ className = "", style }: GalleryRehabilitationyProps) {
  ...
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(rehabImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % rehabImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + rehabImages.length) % rehabImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!rehabImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
