...
export type DiagnostDupuytrenClinicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для діагностичної галереї Dupuytren
const diagnostImages: DiagnostDupuytrenClinicImage[] = [
  {
    src: "/images/hand/diagnostdupuytren1.jpg",
    alt: "Діагностика Dupuytren 1",
    width: 800,
    height: 600,
    title: "Діагностика 1"
  },
  {
    src: "/images/hand/diagnostdupuytren2.jpg",
    alt: "Діагностика Dupuytren 2",
    width: 800,
    height: 600,
    title: "Діагностика 2"
  },
  {
    src: "/images/hand/diagnostdupuytren3.jpg",
    alt: "Діагностика Dupuytren 3",
    width: 800,
    height: 600,
    title: "Діагностика 3"
  },
  {
    src: "/images/hand/diagnostdupuytren4.jpg",
    alt: "Діагностика Dupuytren 4",
    width: 800,
    height: 600,
    title: "Діагностика 4"
  },
...
interface DiagnostDupuytrenClinicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function DiagnostDupuytrenClinic({ className = "", style }: DiagnostDupuytrenClinicProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(diagnostImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % diagnostImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + diagnostImages.length) % diagnostImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!diagnostImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = diagnostImages[currentIndex]
...
export default DiagnostDupuytrenClinic
