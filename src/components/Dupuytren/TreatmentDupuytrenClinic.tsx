...
export type TreatmentDupuytrenClinicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для лікувальної галереї Dupuytren
const treatmentImages: TreatmentDupuytrenClinicImage[] = [
  {
    src: "/images/hand/TreatmentDupuytren1.jpg",
    alt: "Лікування Dupuytren 1",
    width: 800,
    height: 600,
    title: "Лікування 1"
  },
  {
    src: "/images/hand/TreatmentDupuytren2.jpg",
    alt: "Лікування Dupuytren 2",
    width: 800,
    height: 600,
    title: "Лікування 2"
  },
  {
    src: "/images/hand/TreatmentDupuytren3.jpg",
    alt: "Лікування Dupuytren 3",
    width: 800,
    height: 600,
    title: "Лікування 3"
  },
  {
    src: "/images/hand/TreatmentDupuytren4.jpg",
    alt: "Лікування Dupuytren 4",
    width: 800,
    height: 600,
    title: "Лікування 4"
  },
  {
    src: "/images/hand/TreatmentDupuytren5.jpg",
    alt: "Лікування Dupuytren 5",
    width: 800,
    height: 600,
    title: "Лікування 5"
  },
  {
    src: "/images/hand/TreatmentDupuytren6.jpg",
    alt: "Лікування Dupuytren 6",
    width: 800,
    height: 600,
    title: "Лікування 6"
  },
...
interface TreatmentDupuytrenClinicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function TreatmentDupuytrenClinic({ className = "", style }: TreatmentDupuytrenClinicProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(treatmentImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % treatmentImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + treatmentImages.length) % treatmentImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!treatmentImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = treatmentImages[currentIndex]
...
export default TreatmentDupuytrenClinic
