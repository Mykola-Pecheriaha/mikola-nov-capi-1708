...
export type GalleryPhleboStageLaserImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const stageLaserImages: GalleryPhleboStageLaserImage[] = [
  {
    src: "/images/varicose/laser/laser-stage1.jpg",
    alt: "Етап 1 лазерного лікування варикозу",
    width: 800,
    height: 600,
    title: "Розмітка та підготовка"
  },
  {
    src: "/images/varicose/laser/laser-stage4.jpg",
    alt: "Етап 2 лазерного лікування варикозу",
    width: 800,
    height: 600,
    title: "Введення катетера з лазером"
  },
  {
    src: "/images/varicose/laser/laser-stage5.jpg",
    alt: "Етап 3 лазерного лікування варикозу",
    width: 800,
    height: 600,
    title: "Обробка тканин анестетиком"
  },
  {
    src: "/images/varicose/laser/laser-stage6.jpg",
    alt: "Етап 4 лазерного лікування варикозу",
    width: 800,
    height: 600,
...
interface GalleryPhleboStageLaserProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhleboStageLaser({ className = "", style }: GalleryPhleboStageLaserProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(stageLaserImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % stageLaserImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + stageLaserImages.length) % stageLaserImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!stageLaserImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = stageLaserImages[currentIndex]
