...
export type GalleryPhleboReabLaserImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const reabLaserImages: GalleryPhleboReabLaserImage[] = [
  {
    src: "/images/varicose/laser/laser-reab1.jpg",
    alt: "Післяопераційний період 1",
    width: 800,
    height: 600,
    title: "Компресійна білизна"
  },
  {
    src: "/images/varicose/laser/laser-reab2.jpg",
    alt: "Післяопераційний період 2",
    width: 800,
    height: 600,
    title: "Піші прогулянки"
  },
  {
    src: "/images/varicose/laser/laser-reab3.jpg",
    alt: "Післяопераційний період 3",
    width: 800,
    height: 600,
    title: "Легкий масаж"
  },
  {
    src: "/images/varicose/laser/laser-reab.jpg",
    alt: "Післяопераційний період 4",
    width: 800,
    height: 600,
    title: "Фізичне навантаження"
  }
];

interface GalleryPhleboReabLaserProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhleboReabLaser({ className = "", style }: GalleryPhleboReabLaserProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(reabLaserImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % reabLaserImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + reabLaserImages.length) % reabLaserImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!reabLaserImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = reabLaserImages[currentIndex];
