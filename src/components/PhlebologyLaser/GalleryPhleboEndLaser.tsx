...
export type GalleryPhleboEndLaserImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const endLaserImages: GalleryPhleboEndLaserImage[] = [
  {
    src: "/images/varicose/laser/end-laser.jpg",
    alt: "Лазерна коагуляція 1",
    width: 800,
    height: 600,
    title: "Безболісність та безпека"
  },
  {
    src: "/images/varicose/laser/end-laser1.jpg",
    alt: "Лазерна коагуляція 2",
    width: 800,
    height: 600,
    title: "Швидке проведення операції"
  },
  {
    src: "/images/varicose/laser/end-laser2.jpg",
    alt: "Лазерна коагуляція 3",
    width: 800,
    height: 600,
    title: "Ефективність"
  },
  {
    src: "/images/varicose/laser/end-laser5.jpg",
    alt: "Лазерна коагуляція 4",
    width: 800,
    height: 600,
    title: "Косметичний ефект"
  }
];

interface GalleryPhleboEndLaserProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPhleboEndLaser({ className = "", style }: GalleryPhleboEndLaserProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(endLaserImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % endLaserImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + endLaserImages.length) % endLaserImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!endLaserImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = endLaserImages[currentIndex];
