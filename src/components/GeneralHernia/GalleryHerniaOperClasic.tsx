...
export type GalleryHerniaOperClasicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const operClasicImages: GalleryHerniaOperClasicImage[] = [
  { src: "/images/general-hernia/gernioPreparation10.jpg", alt: "Класична операція 1", width: 800, height: 600, title: "імплантація сітки" },
  { src: "/images/general-hernia/gernioPreparation11.jpg", alt: "Класична операція 1", width: 800, height: 600, title: "імплантація сітки" },

  { src: "/images/general-hernia/operclasic1.jpg", alt: "Класична операція 1", width: 800, height: 600, title: "імплантація сітки" },
  { src: "/images/general-hernia/operclasic2.jpg", alt: "Класична операція 2", width: 800, height: 600, title: "Види гриж " },
  { src: "/images/general-hernia/operclasic3.jpg", alt: "Класична операція 3", width: 800, height: 600, title: "Пахова грижа" },
  { src: "/images/general-hernia/operclasic4.jpg", alt: "Класична операція 4", width: 800, height: 600, title: "Вентральна грижа" },
  { src: "/images/general-hernia/operclasic5.jpg", alt: "Класична операція 5", width: 800, height: 600, title: "Вентральна грижа імплантованою сіткою " },

];

interface GalleryHerniaOperClasicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryHerniaOperClasic({ className = "", style }: GalleryHerniaOperClasicProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(operClasicImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % operClasicImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + operClasicImages.length) % operClasicImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!operClasicImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = operClasicImages[currentIndex];
