...
export type GalleryHerniaStartImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const herniaImages: GalleryHerniaStartImage[] = [
  { src: "/images/general-hernia/gerniyaі.jpg", alt: "Грижа черевної стінки 1", width: 800, height: 600, title: "Особливості анатомічної будови" },
  { src: "/images/general-hernia/gerniya2.jpg", alt: "Грижа черевної стінки 2", width: 800, height: 600, title: "закрепи" },
  { src: "/images/general-hernia/gerniya4.jpg", alt: "Грижа черевної стінки 3", width: 800, height: 600, title: "захворювання із сильним кашлем;" },
  { src: "/images/general-hernia/gerniya5.jpg", alt: "Грижа черевної стінки 4", width: 800, height: 600, title: "порушення сечовипускання" },
  { src: "/images/general-hernia/gerniya6.jpg", alt: "Грижа черевної стінки 5", width: 800, height: 600, title: "тренажерному залі" },
  { src: "/images/general-hernia/gerniya7.jpg", alt: "Грижа черевної стінки 6", width: 800, height: 600, title: " жінок – вагітність" },
  { src: "/images/general-hernia/gerniya8.jpg", alt: "Грижа черевної стінки 7", width: 800, height: 600, title: "Грижа 7" },
  { src: "/images/general-hernia/gerniya9.jpg", alt: "Грижа черевної стінки 8", width: 800, height: 600, title: "пухлини черевної порожнини" },
    { src: "/images/general-hernia/gerniya10.jpg", alt: "Грижа черевної стінки 8", width: 800, height: 600, title: "ожиріння" },
];

interface GalleryHerniaStartProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryHerniaStart({ className = "", style }: GalleryHerniaStartProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(herniaImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % herniaImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + herniaImages.length) % herniaImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!herniaImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = herniaImages[currentIndex];
