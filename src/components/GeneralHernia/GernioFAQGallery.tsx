...
export function GernioFAQGallery({ images = [] }: GernioFAQGalleryProps) {
  ...
  const nextImage = () => safeImages.length && setCurrentIndex(prev => (prev + 1) % safeImages.length);
  const prevImage = () => safeImages.length && setCurrentIndex(prev => (prev - 1 + safeImages.length) % safeImages.length);
  const openFullscreen = () => setFullscreen(true);
  const closeFullscreen = () => { setFullscreen(false); setShowThumbnails(false); };
  const toggleThumbnails = () => setShowThumbnails(s => !s);
  const selectImage = (idx: number) => idx >= 0 && idx < safeImages.length && setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    if (idx >= 0 && idx < imagesLoaded.length) {
  ...
  return (
    <div className="w-full relative">
      <div className="relative w-full h-[400px] bg-white rounded-lg overflow-hidden shadow-lg">
        <button className="absolute top-1/2 -translate-y-1/2 left-4 w-9 h-9 bg-white/90 border-none rounded-full flex items-center justify-center text-lg cursor-pointer transition-all duration-300 z-30 text-green-600 hover:bg-white hover:scale-110" onClick={prevImage} aria-label="Попереднє зображення">&#10094;</button>
        ...
        <button className="absolute top-1/2 -translate-y-1/2 right-4 w-9 h-9 bg-white/90 border-none rounded-full flex items-center justify-center text-lg cursor-pointer transition-all duration-300 z-30 text-green-600 hover:bg-white hover:scale-110" onClick={nextImage} aria-label="Наступне зображення">&#10095;</button>
        <div className="absolute bottom-0 left-0 right-0 h-11 bg-white flex items-center justify-between px-4 z-20">
          <div className="flex justify-center items-center gap-2 flex-1">
            {safeImages.map((_, idx) => (
              <span key={idx} className={`w-2 h-2 rounded-full bg-gray-300 cursor-pointer transition-all duration-300 ${idx === currentIndex ? 'bg-green-600 scale-125' : ''}`} onClick={() => selectImage(idx)} />
            ))}
          </div>
          <button className="w-7 h-7 bg-transparent border border-gray-300 rounded flex items-center justify-center text-base cursor-pointer transition-all duration-300 text-green-600 font-bold hover:bg-green-600 hover:text-white hover:border-green-600" onClick={openFullscreen} aria-label="Відкрити на повний екран">+</button>
        </div>
      </div>
      {fullscreen && (
        <div className="fixed top-0 left-0 w-full h-full bg-black/90 flex items-center justify-center z-[1000]" onClick={closeFullscreen}>
          <div className="relative w-[90%] h-[90%] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            <button className="absolute top-5 right-5 w-10 h-10 bg-white/20 border-none rounded-full text-white text-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:bg-white/30" onClick={closeFullscreen} aria-label="Закрити">&times;</button>
            <button className="absolute top-5 right-20 w-10 h-10 bg-white/20 border-none rounded-full text-white text-2xl flex items-center justify-center cursor-pointer transition-all duration-300 hover:bg-white/30" onClick={toggleThumbnails} aria-label="Показати мініатюри">&#8942;</button>
