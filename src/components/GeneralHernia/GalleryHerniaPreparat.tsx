...
export type GalleryHerniaPreparatImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const preparatImages: GalleryHerniaPreparatImage[] = [
  { src: "/images/general-hernia/gerniya16.jpg", alt: "Підготовка до операції 1", width: 800, height: 600, title: "Підготовка 1" },
  { src: "/images/general-hernia/gerniya17.jpg", alt: "Підготовка до операції 2", width: 800, height: 600, title: "Підготовка 2" },
  { src: "/images/general-hernia/gerniya18.jpg", alt: "Підготовка до операції 3", width: 800, height: 600, title: "Підготовка 3" },
  { src: "/images/general-hernia/gerniya19.jpg", alt: "Підготовка до операції 4", width: 800, height: 600, title: "Підготовка 4" },


];

interface GalleryHerniaPreparatProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryHerniaPreparat({ className = "", style }: GalleryHerniaPreparatProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(preparatImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % preparatImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + preparatImages.length) % preparatImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!preparatImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = preparatImages[currentIndex];
