...
export type GalleryProctoImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї проктології
const proctoImages: GalleryProctoImage[] = [
  {
    src: "/images/procto/proctology.jpg",
    alt: "Проктологія 1",
    width: 800,
    height: 600,
  ...
  {
    src: "/images/procto/proctology1.jpg",
    alt: "Проктологія 2",
    width: 800,
    height: 600,
...
interface GalleryProctoProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryProcto({ className = "", style }: GalleryProctoProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(proctoImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % proctoImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + proctoImages.length) % proctoImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!proctoImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = proctoImages[currentIndex]
