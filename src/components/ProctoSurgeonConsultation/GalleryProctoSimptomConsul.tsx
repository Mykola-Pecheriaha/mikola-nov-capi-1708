...
export type GalleryProctoSimptomConsulImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї симптомів (8 шт)
const simptomImages: GalleryProctoSimptomConsulImage[] = [
  { src: "/images/procto/proctology21.jpg", alt: "Симптом 1", width: 800, height: 600, title: "гострі або тупі болі в області промежини та ануса." },
  { src: "/images/procto/proctology22.jpg", alt: "Симптом 2", width: 800, height: 600, title: "виділення крові," },
  { src: "/images/procto/proctology24.jpg", alt: "Симптом 3", width: 800, height: 600, title: "поява новоутворень" },
  { src: "/images/procto/proctology26.jpg", alt: "Симптом 4", width: 800, height: 600, title: "відчуття сверблячки та печіння в зоні ануса" },
  { src: "/images/procto/proctology27.jpg", alt: "Симптом 5", width: 800, height: 600, title: "хронічні запори чи проноси" },
  { src: "/images/procto/proctology29.jpg", alt: "Симптом 6", width: 800, height: 600, title: "стороннє тіло у прямій кишці" },
  { src: "/images/procto/proctology25.jpg", alt: "Симптом 7", width: 800, height: 600, title: "вихід з ануса гемороїдальних шишок" },
  { src: "/images/procto/proctology28.jpg", alt: "Симптом 8", width: 800, height: 600, title: "травматичні ушкодження заднього проходу" },
];

interface GalleryProctoSimptomConsulProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryProctoSimptomConsul({ className = "", style }: GalleryProctoSimptomConsulProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(simptomImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % simptomImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + simptomImages.length) % simptomImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!simptomImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = simptomImages[currentIndex];
