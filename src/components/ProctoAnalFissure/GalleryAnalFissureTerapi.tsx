...
export type GalleryAnalFissureTerapiImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const terapiImages: GalleryAnalFissureTerapiImage[] = [
  { src: "/images/procto/fissure/analFissure14.jpg", alt: "Лікування анальної тріщини 1", width: 800, height: 600, title: "Консервативне лікування" },
  { src: "/images/procto/fissure/analFissure18.jpg", alt: "Лікування анальної тріщини 2", width: 800, height: 600, title: "Операція початок" },
  { src: "/images/procto/fissure/analFissure19.jpg", alt: "Лікування анальної тріщини 3", width: 800, height: 600, title: "Видалення" },
  { src: "/images/procto/fissure/analFissure20.jpg", alt: "Лікування анальної тріщини 4", width: 800, height: 600, title: "Завершення операції" },
  { src: "/images/procto/fissure/fissure31jpg.jpg", alt: "Лікування анальної тріщини 5", width: 800, height: 600, title: "Лікування лазером" },
  { src: "/images/procto/fissure/fissure32jpg.jpg", alt: "Лікування анальної тріщини 6", width: 800, height: 600, title: "Консервативне лікування" },
];

interface GalleryAnalFissureTerapiProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryAnalFissureTerapi({ className = "", style }: GalleryAnalFissureTerapiProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(terapiImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % terapiImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + terapiImages.length) % terapiImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!terapiImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = terapiImages[currentIndex];
