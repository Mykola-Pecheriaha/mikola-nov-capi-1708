...
export type GalleryAnalFissureComplicImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const complicImages: GalleryAnalFissureComplicImage[] = [
  { src: "/images/procto/fissure/fissure30jpg.jpg", alt: "Ускладнення анальної тріщини 1", width: 800, height: 600, title: "Хронізація процесу." },
  { src: "/images/procto/fissure/fissure27jpg.jpg", alt: "Ускладнення анальної тріщини 2", width: 800, height: 600, title: "Розвиток гострого парапроктиту" },
  { src: "/images/procto/fissure/fissure28jpg.jpg", alt: "Ускладнення анальної тріщини 3", width: 800, height: 600, title: "Утворення нориць-хронічний парапроктит" },
  { src: "/images/procto/fissure/fissure7.jpg", alt: "Ускладнення анальної тріщини 4", width: 800, height: 600, title: "Кровотечі" },

];

interface GalleryAnalFissureComplicProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryAnalFissureComplic({ className = "", style }: GalleryAnalFissureComplicProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(complicImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % complicImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + complicImages.length) % complicImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!complicImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = complicImages[currentIndex];
