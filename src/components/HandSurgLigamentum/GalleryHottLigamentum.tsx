...
export type GalleryHottLigamentumImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const hottImages: GalleryHottLigamentumImage[] = [
  {
    src: "/images/hand/hott1.jpg",
    alt: "Стенозуючий лігаментит 1",
    width: 800,
    height: 600,
    title: "Стадія 1"
  },
  {
    src: "/images/hand/hott2.jpg",
    alt: "Стенозуючий лігаментит 2",
    width: 800,
    height: 600,
    title: "Стадія 2"
  },
  {
    src: "/images/hand/hott3.jpg",
    alt: "Стенозуючий лігаментит 3",
    width: 800,
    height: 600,
    title: "Стадія 3"
  },
  {
    src: "/images/hand/hott4.jpg",
    alt: "Стенозуючий лігаментит 4",
    width: 800,
    height: 600,
    title: "Стадія 4"
  },
    {
    src: "/images/hand/hott5.jpg",
    alt: "Стенозуючий лігаментит 4",
    width: 800,
    height: 600,
    title: "Стадія 4"
...
const GalleryHottLigamentum = ({ className = "", style }: { className?: string; style?: React.CSSProperties }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(hottImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % hottImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + hottImages.length) % hottImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!hottImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = hottImages[currentIndex];
...
export default GalleryHottLigamentum;
