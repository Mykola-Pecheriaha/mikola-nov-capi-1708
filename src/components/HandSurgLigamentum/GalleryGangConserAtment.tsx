...
export type GalleryGangConserImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const gangConserImages: GalleryGangConserImage[] = [
  {
    src: "/images/hand/gigroma/gigroma1.jpg",
    alt: "Гігрома консервативне 1",
    width: 800,
    height: 600,
    title: "До/Після лікування 1"
  },
  {
    src: "/images/hand/gigroma/gigroma2.jpg",
    alt: "Гігрома консервативне 2",
    width: 800,
    height: 600,
    title: "До/Після лікування 2"
  },
  {
    src: "/images/hand/gigroma/gigroma3.jpg",
    alt: "Гігрома консервативне 3",
    width: 800,
    height: 600,
    title: "До/Після лікування 3"
  },
  {
    src: "/images/hand/gigroma/gigroma4.jpg",
    alt: "Гігрома консервативне 4",
    width: 800,
    height: 600,
    title: "До/Після лікування 4"
  }
];

interface GalleryGangConserAtmentProps {
  className?: string;
  style?: React.CSSProperties;
}

const GalleryGangConserAtment: React.FC<GalleryGangConserAtmentProps> = ({ className = "", style }) => {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(gangConserImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % gangConserImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + gangConserImages.length) % gangConserImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!gangConserImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = gangConserImages[currentIndex]
...
export default GalleryGangConserAtment;
