...
export type GalleryNottaAtmentImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const nottaImages: GalleryNottaAtmentImage[] = [
  {
    src: "/images/hand/notta-atm/notta-atm1.jpg",
    alt: "Хвороба Нотта 1",
    width: 800,
    height: 600,
    title: "Стадія 1"
  ...
  {
    src: "/images/hand/notta-atm/notta-atm2.jpg",
    alt: "Хвороба Нотта 2",
    width: 800,
    height: 600,
    title: "Стадія 2"
  ...
  {
    src: "/images/hand/notta-atm/notta-atm3.jpg",
    alt: "Хвороба Нотта 3",
    width: 800,
    height: 600,
    title: "Стадія 3"
  ...
  {
    src: "/images/hand/notta-atm/notta-atm4.jpg",
    alt: "Хвороба Нотта 4",
    width: 800,
    height: 600,
    title: "Стадія 4"
  ...
  {
    src: "/images/hand/notta-atm/notta-atm6.jpg",
    alt: "Хвороба Нотта 5",
    width: 800,
    height: 600,
    title: "Стадія 5"
    ...
    {
    src: "/images/hand/notta-atm/notta-atm7.jpg",
    alt: "Хвороба Нотта 5",
    width: 800,
    height: 600,
    title: "Стадія 5"
...
const GalleryNottaAtment = ({ className = "", style }: { className?: string; style?: React.CSSProperties }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(nottaImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % nottaImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + nottaImages.length) % nottaImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!nottaImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = nottaImages[currentIndex];
...
export default GalleryNottaAtment;
