...
const GalleryGangLigamentum = ({ className = "", style }: { className?: string; style?: React.CSSProperties }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(gangImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % gangImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + gangImages.length) % gangImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!gangImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = gangImages[currentIndex];
...
export default GalleryGangLigamentum;
