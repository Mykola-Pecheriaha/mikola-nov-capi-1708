...
export type GalleryGangReabImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const gangReabImages: GalleryGangReabImage[] = [
  {
    src: "/images/hand/gigroma/gigroma11.jpg",
    alt: "Гігрома реабілітація 1",
    width: 800,
    height: 600,
    title: "Реабілітація 1"
  },
  {
    src: "/images/hand/gigroma/gigroma12.jpg",
    alt: "Гігрома реабілітація 2",
    width: 800,
    height: 600,
    title: "Реабілітація 2"
  },
  {
    src: "/images/hand/gigroma/gigroma14.jpg",
    alt: "Гігрома реабілітація 3",
    width: 800,
    height: 600,
    title: "Реабілітація 3"
  },
  {
    src: "/images/hand/gigroma/carpale20.jpg",
    alt: "Гігрома реабілітація 4",
    width: 800,
    height: 600,
    title: "Реабілітація 4"
  }
];

interface GalleryGangReabAtmentProps {
  className?: string;
  style?: React.CSSProperties;
}

const GalleryGangReabAtment: React.FC<GalleryGangReabAtmentProps> = ({ className = "", style }) => {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(gangReabImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % gangReabImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + gangReabImages.length) % gangReabImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!gangReabImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = gangReabImages[currentIndex]
...
export default GalleryGangReabAtment;
