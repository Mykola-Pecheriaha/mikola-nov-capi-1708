...
export type GalleryGangOperImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const gangOperImages: GalleryGangOperImage[] = [
  {
    src: "/images/hand/gigroma/gigroma8.jpg",
    alt: "Гігрома операція 1",
    width: 800,
    height: 600,
    title: "Операція 1"
  },
  {
    src: "/images/hand/gigroma/gigroma13.jpg",
    alt: "Гігрома операція 2",
    width: 800,
    height: 600,
    title: "Операція 2"
  },
  {
    src: "/images/hand/gigroma/gigroma9.jpg",
    alt: "Гігрома операція 3",
    width: 800,
    height: 600,
    title: "Операція 3"
  },
  {
    src: "/images/hand/gigroma/gigroma5.jpg",
    alt: "Гігрома операція 4",
    width: 800,
    height: 600,
    title: "Операція 4"
  }
...
interface GalleryGangOperAtmentProps {
  className?: string;
  style?: React.CSSProperties;
}

const GalleryGangOperAtment: React.FC<GalleryGangOperAtmentProps> = ({ className = "", style }) => {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(gangOperImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % gangOperImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + gangOperImages.length) % gangOperImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!gangOperImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = gangOperImages[currentIndex]
...
export default GalleryGangOperAtment;
