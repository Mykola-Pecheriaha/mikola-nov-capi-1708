...
export type GalleryPararectalFistulaReabilImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const reabilImages: GalleryPararectalFistulaReabilImage[] = [
  { src: "/images/procto/pararectal-fistula/parafistula13jpg.jpg", alt: "Реабілітація після видалення нориці 1", width: 800, height: 600, title: "Реабілітація 1" },

];

interface GalleryPararectalFistulaReabilProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryPararectalFistulaReabil({ className = "", style }: GalleryPararectalFistulaReabilProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(reabilImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % reabilImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + reabilImages.length) % reabilImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!reabilImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = reabilImages[currentIndex];
