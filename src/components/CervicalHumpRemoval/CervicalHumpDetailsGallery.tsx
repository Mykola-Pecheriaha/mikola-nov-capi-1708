...
interface CervicalHumpDetailsGalleryProps {
  images: GalleryImage[];
}

export function CervicalHumpDetailsGallery({ images }: CervicalHumpDetailsGalleryProps) {
    ...
    <div className="w-full mx-auto relative">
      <div className="relative w-full h-96 bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md border border-blue-100 dark:border-blue-900">
        ...
        {images.length > 1 && (
          <>
            <button
              className="absolute top-1/2 left-4 -translate-y-1/2 w-9 h-9 bg-white/10 dark:bg-gray-700/10 border-none rounded-full flex items-center justify-center text-lg cursor-pointer transition-all duration-300 z-10 text-blue-500 dark:text-blue-400 shadow-md hover:bg-blue-500 hover:dark:bg-blue-600 hover:text-white hover:scale-110"
              onClick={prevImage}
            ...
            <button
              className="absolute top-1/2 right-4 -translate-y-1/2 w-9 h-9 bg-white/10 dark:bg-gray-700/10 border-none rounded-full flex items-center justify-center text-lg cursor-pointer transition-all duration-300 z-10 text-blue-500 dark:text-blue-400 shadow-md hover:bg-blue-500 hover:dark:bg-blue-600 hover:text-white hover:scale-110"
              onClick={nextImage}
              ...
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2 sm:gap-3 p-3 bg-black/60 overflow-x-auto">
                {images.map((image, index) => (
                  ...
                  >
                    <Image
                      src={
                        image.src || '/placeholder.svg?height=75&width=100&query=cervical+thumbnail'
                      }
                      alt={image.alt}
                      width={100}
                      height={75}
...
export default CervicalHumpDetailsGallery;
