...
export type GalleryPhleboConsultImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї консультації флеболога
const phleboImages: GalleryPhleboConsultImage[] = [
  {
    src: "/images/varicose/phleboConsult.jpg",
    alt: "Флеболог консультація 1",
    width: 800,
    height: 600,
    title: "Варикоз"
  },
  {
    src: "/images/varicose/phleboConsult1.jpg",
    alt: "Флеболог консультація 2",
    width: 800,
    height: 600,
    title: "Варикоз"
  },
  {
    src: "/images/varicose/phleboConsult3.jpg",
    alt: "Флеболог консультація 3",
    width: 800,
    height: 600,
    title: "Розрив варикозної вени"
  },
  {
    src: "/images/varicose/phleboConsult4.jpg",
    alt: "Флеболог консультація 4",
    width: 800,
    height: 600,
    title: "Венозна сітка"
  },
    {
    src: "/images/varicose/phleboConsult6.jpg",
    alt: "Флеболог консультація 4",
    width: 800,
    height: 600,
    title: "ККомпресійна білизна"
  }
  
];

interface GalleryPhleboConsultProps {
  className?: string;
  style?: React.CSSProperties;
}

export default function GalleryPhleboConsult({ className = "", style }: GalleryPhleboConsultProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(phleboImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % phleboImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + phleboImages.length) % phleboImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!phleboImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = phleboImages[currentIndex]
