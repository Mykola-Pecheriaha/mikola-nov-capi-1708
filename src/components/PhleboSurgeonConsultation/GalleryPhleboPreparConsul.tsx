...
export type GalleryPhleboPreparConsulImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const phleboPreparImages: GalleryPhleboPreparConsulImage[] = [
  {
    src: "/images/varicose/varicos35.jpg",
    alt: "Підготовка 1",
    width: 800,
    height: 600,
  ...
  {
    src: "/images/varicose/varicos36.jpg",
    alt: "Підготовка 2",
    width: 800,
    height: 600,
    title: "Узд судин"
  ...
  {
    src: "/images/varicose/varicos37.jpg",
    alt: "Підготовка 3",
    width: 800,
    height: 600,
    title: "Доплерографія"
...
export default function GalleryPhleboPreparConsul({ className = "", style }: { className?: string; style?: React.CSSProperties } = {}) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(phleboPreparImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % phleboPreparImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + phleboPreparImages.length) % phleboPreparImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!phleboPreparImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = phleboPreparImages[currentIndex];
          ...
          <div className="relative w-[90%] h-[90%] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            ...
            {showThumbnails && (
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2 p-2 bg-black/60 overflow-x-auto">
                {phleboPreparImages.map((img, idx) => (
