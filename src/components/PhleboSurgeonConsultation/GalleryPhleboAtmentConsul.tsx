...
export type GalleryPhleboAtmentConsulImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї лікування (8 шт)
const atmentImages: GalleryPhleboAtmentConsulImage[] = [
  {
...
interface GalleryPhleboAtmentConsulProps {
  className?: string;
  style?: React.CSSProperties;
}

export default function GalleryPhleboAtmentConsul({ className = "", style }: GalleryPhleboAtmentConsulProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(atmentImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % atmentImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + atmentImages.length) % atmentImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!atmentImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = atmentImages[currentIndex]
    ...
    <div className={`w-full relative ${className}`} style={style}>
      <div className="relative w-full h-[400px] bg-white rounded-lg overflow-hidden shadow-lg">
        ...
        <div className="relative w-[calc(100%-8px)] h-[calc(100%-48px)] m-1 mx-1 mb-11 overflow-hidden rounded">
          ...
          <Image src={currentImage.src} alt={currentImage.alt.replace('"', '&quot;').replace("'", "&#39;")} width={currentImage.width} height={currentImage.height} className="w-full h-full object-cover transition-transform duration-300 hover:scale-105" onLoad={() => handleImageLoad(currentIndex)} onError={() => handleImageLoad(currentIndex)} priority={currentIndex === 0} />
        </div>
