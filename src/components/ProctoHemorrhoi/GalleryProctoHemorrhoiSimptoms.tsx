...
export type GalleryProctoHemorrhoiSimptomsImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї симптомів геморою (4 шт)
const simptomsImages: GalleryProctoHemorrhoiSimptomsImage[] = [
  { src: "/images/procto/hemorroi/hemorroi6.jpg", alt: "Симптом 1", width: 800, height: 600, title: "Симптом 1" },
  { src: "/images/procto/hemorroi/hemorroi8.jpg", alt: "Симптом 2", width: 800, height: 600, title: "Симптом 2" },
  { src: "/images/procto/hemorroi/hemorroi9.jpg", alt: "Симптом 3", width: 800, height: 600, title: "Симптом 3" },
  { src: "/images/procto/hemorroi/hemorroi11.jpg", alt: "Симптом 4", width: 800, height: 600, title: "Симптом 4" },
    { src: "/images/procto/hemorroi/hemorroi12.jpg", alt: "Симптом 4", width: 800, height: 600, title: "Симптом 4" },
      { src: "/images/procto/hemorroi/hemorroi17.jpg", alt: "Симптом 4", width: 800, height: 600, title: "Симптом 4" },
];

interface GalleryProctoHemorrhoiSimptomsProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryProctoHemorrhoiSimptoms({ className = "", style }: GalleryProctoHemorrhoiSimptomsProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(simptomsImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % simptomsImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + simptomsImages.length) % simptomsImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!simptomsImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = simptomsImages[currentIndex];
