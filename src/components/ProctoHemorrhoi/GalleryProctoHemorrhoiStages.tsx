...
export type GalleryProctoHemorrhoiStagesImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї стадій геморою (4 шт)
const stagesImages: GalleryProctoHemorrhoiStagesImage[] = [
  { src: "/images/procto/hemorroi/hemorroi2.jpg", alt: "Стадія 1", width: 800, height: 600, title: "I ступінь" },
  { src: "/images/procto/hemorroi/hemorroi3.jpg", alt: "Стадія 2", width: 800, height: 600, title: "II ступінь" },
  { src: "/images/procto/hemorroi/hemorroi4.jpg", alt: "Стадія 3", width: 800, height: 600, title: "III ступінь" },
  { src: "/images/procto/hemorroi/hemorroi5.jpg", alt: "Стадія 4", width: 800, height: 600, title: "IV ступінь" },
];

interface GalleryProctoHemorrhoiStagesProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryProctoHemorrhoiStages({ className = "", style }: GalleryProctoHemorrhoiStagesProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(stagesImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % stagesImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + stagesImages.length) % stagesImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!stagesImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = stagesImages[currentIndex];
