...
export type GalleryCarpalSymptomsImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const carpalImages: GalleryCarpalSymptomsImage[] = [
  {
    src: "/images/hand/carpale-details/carpale16.jpg",
    alt: "Симптом 1",
    width: 800,
    height: 600,
    title: "Симптом 1"
  ...
  {
    src: "/images/hand/carpale-details/carpale27.jpg",
    alt: "Симптом 2",
    width: 800,
    height: 600,
    title: "Симптом 2"
  ...
  {
    src: "/images/hand/carpale-details/carpal5.jpg",
    alt: "Симптом 3",
    width: 800,
    height: 600,
    title: "Симптом 3"
  ...
  {
    src: "/images/hand/carpale-details/carp39.jpg",
    alt: "Симптом 4",
    width: 800,
    height: 600,
    title: "Симптом 4"
  ...
  {
    src: "/images/hand/carpale-details/carpale29.jpg",
    alt: "Симптом 5",
    width: 800,
    height: 600,
    title: "Симптом 5"
...
export default function GalleryCarpalSymptoms({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(carpalImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % carpalImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + carpalImages.length) % carpalImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!carpalImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = carpalImages[currentIndex];
