...
export type GalleryCarpalConserTreatmImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const carpalConserImages: GalleryCarpalConserTreatmImage[] = [
  {
    src: "/images/hand/сarpal-terapi/carpale34.jpg",
    alt: "Консервативне лікування 1",
    width: 800,
    height: 600,
    title: "Консервативне лікування 1"
  },
  {
    src: "/images/hand/сarpal-terapi/gigroma14.jpg",
    alt: "Консервативне лікування 2",
    width: 800,
    height: 600,
    title: "Консервативне лікування 2"
  },
  {
    src: "/images/hand/сarpal-terapi/carpale43.jpg",
    alt: "Консервативне лікування 3",
    width: 800,
    height: 600,
    title: "Консервативне лікування 3"
  },
  {
    src: "/images/hand/сarpal-terapi/carpale40.jpg",
    alt: "Консервативне лікування 4",
    width: 800,
    height: 600,
    title: "Консервативне лікування 4"
  },

];

export default function GalleryCarpalConserTreatm({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(carpalConserImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % carpalConserImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + carpalConserImages.length) % carpalConserImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!carpalConserImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = carpalConserImages[currentIndex];
          ...
          <div className="relative w-[90%] h-[90%] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            ...
            {showThumbnails && (
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2 p-2 bg-black/60 overflow-x-auto">
                {carpalConserImages.map((img, idx) => (
