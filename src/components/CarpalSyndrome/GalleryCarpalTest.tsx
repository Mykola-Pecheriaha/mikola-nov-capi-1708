...
export type GalleryCarpalTestImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const carpalTestImages: GalleryCarpalTestImage[] = [
  {
    src: "/images/hand/carpale-details/carpale30.jpg",
    alt: "Симптом 1",
    width: 800,
    height: 600,
    title: "Симптом 1"
  ...
  {
    src: "/images/hand/carpale-details/carpale31.jpg",
    alt: "Симптом 2",
    width: 800,
    height: 600,
    title: "Симптом 2"
  ...
  {
    src: "/images/hand/carpale-details/carpale32.jpg",
    alt: "Симптом 3",
    width: 800,
    height: 600,
    title: "Симптом 3"
  ...
  {
    src: "/images/hand/carpale-details/carpale33.jpg",
    alt: "Симптом 4",
    width: 800,
    height: 600,
    title: "Симптом 4"
...
export default function GalleryCarpalTest({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(carpalTestImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % carpalTestImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + carpalTestImages.length) % carpalTestImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!carpalTestImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = carpalTestImages[currentIndex];
