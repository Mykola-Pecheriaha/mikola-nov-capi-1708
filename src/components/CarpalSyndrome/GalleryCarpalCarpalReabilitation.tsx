...
export type GalleryCarpalCarpalReabilitationImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї реабілітації
const reabilitationImages: GalleryCarpalCarpalReabilitationImage[] = [
  {
    src: "/images/hand/carpal-reabil/carp15.jpg",
    alt: "Реабілітація 1",
    width: 800,
    height: 600,
    title: "Вправи після операції 1"
  },
  {
    src: "/images/hand/carpal-reabil/carp41.jpg",
    alt: "Реабілітація 2",
    width: 800,
    height: 600,
    title: "Вправи після операції 2"
  },
  {
    src: "/images/hand/carpal-reabil/carpale46.jpg",
    alt: "Реабілітація 3",
    width: 800,
    height: 600,
    title: "Вправи після операції 3"
  },
  {
    src: "/images/hand/carpal-reabil/carpale51.jpg",
    alt: "Реабілітація 4",
    width: 800,
    height: 600,
    title: "Вправи після операції 4"
  }
];

interface GalleryCarpalCarpalReabilitationProps {
  className?: string;
  style?: React.CSSProperties;
}

export default function GalleryCarpalCarpalReabilitation({ className = "", style }: GalleryCarpalCarpalReabilitationProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(reabilitationImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % reabilitationImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + reabilitationImages.length) % reabilitationImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!reabilitationImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = reabilitationImages[currentIndex]
          ...
          <div className="relative w-[90%] h-[90%] flex items-center justify-center" onClick={e => e.stopPropagation()}>
            ...
            {showThumbnails && (
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2 p-2 bg-black/60 overflow-x-auto">
                {reabilitationImages.map((img, idx) => (
