...
export type GalleryCarpalResultImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї результатів
const resultImages: GalleryCarpalResultImage[] = [
  {
  ...
  {
    src: "/images/hand/carpal-reabil/carpale49.jpg",
    alt: "Результат 2",
    width: 800,
    height: 600,
  ...
  {
    src: "/images/hand/carpal-reabil/carpale50.jpg",
    alt: "Результат 3",
    width: 800,
    height: 600,
  ...
  {
    src: "/images/hand/carpal-reabil/carpale52.jpg",
    alt: "Результат 4",
    width: 800,
    height: 600,
...
interface GalleryCarpalResultProps {
  className?: string;
  style?: React.CSSProperties;
}

export default function GalleryCarpalResult({ className = "", style }: GalleryCarpalResultProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(resultImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % resultImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + resultImages.length) % resultImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!resultImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = resultImages[currentIndex]
