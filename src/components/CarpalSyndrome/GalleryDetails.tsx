...
const images = [
  { src: "/images/hand/carpale-details/carpal7.jpg", alt: "Карпальний синдром 1", width: 800, height: 600, title: "Зображення 1" },
  { src: "/images/hand/carpale-details/carpale20.jpg", alt: "Карпальний синдром 2", width: 800, height: 600, title: "Зображення 2" },
  { src: "/images/hand/carpale-details/carpale25.jpg", alt: "Карпальний ,синдром 3", width: 800, height: 600, title: "Зображення 3" },
    { src: "/images/hand/carpale-details/carpale31.jpg", alt: "Карпальний ,синдром 3", width: 800, height: 600, title: "Зображення 3" },
      { src: "/images/hand/carpale-details/carpale26.jpg", alt: "Карпальний ,синдром 3", width: 800, height: 600, title: "Зображення 3" },
];

const GalleryDetails = ({ className = "", style }: { className?: string; style?: React.CSSProperties }) => {
  ...
  if (!images.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
              ...
              <div className="absolute bottom-5 left-0 right-0 flex justify-center gap-2 p-2 bg-black/60 overflow-x-auto">
                {images.map((img, idx) => (
                  <div key={idx} className={`w-24 h-20 border-2 rounded cursor-pointer overflow-hidden transition-all duration-300 ${idx === currentIndex ? 'border-white' : 'border-transparent'}`} onClick={() => selectImage(idx)}>
                    <Image src={img.src} alt={img.alt} width={img.width} height={img.height} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            )}
