...
export type GalleryCarpalOperTreatmImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const carpalOperImages: GalleryCarpalOperTreatmImage[] = [
  {
    src: "/images/hand/сarpal-terapi/carpal2.jpg",
    alt: "Оперативне лікування 1",
    width: 800,
    height: 600,
    title: "Оперативне лікування 1"
  },
  {
    src: "/images/hand/сarpal-terapi/carpal3.jpg",
    alt: "Оперативне лікування 2",
    width: 800,
    height: 600,
    title: "Оперативне лікування 2"
  },
  {
    src: "/images/hand/сarpal-terapi/carpal4.jpg",
    alt: "Оперативне лікування 3",
    width: 800,
    height: 600,
    title: "Оперативне лікування 3"
  },
  {
    src: "/images/hand/сarpal-terapi/carp16.jpg",
    alt: "Оперативне лікування 4",
    width: 800,
    height: 600,
    title: "Оперативне лікування 4"
  },
  {
    src: "/images/hand/сarpal-terapi/carpale22.jpg",
    alt: "Оперативне лікування 5",
    width: 800,
    height: 600,
    title: "Оперативне лікування 5"
  },
  {
    src: "/images/hand/сarpal-terapi/carpale45.jpg",
    alt: "Оперативне лікування 6",
    width: 800,
    height: 600,
    title: "Оперативне лікування 6"
  },
];

export default function GalleryCarpalOperTreatm({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(carpalOperImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % carpalOperImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + carpalOperImages.length) % carpalOperImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!carpalOperImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = carpalOperImages[currentIndex];
