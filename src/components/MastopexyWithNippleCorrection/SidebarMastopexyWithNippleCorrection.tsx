...
export default function SidebarMastopexyWithNippleCorrection() {
        ...
        <ul className="space-y-3 text-base font-medium">
          {menu
            .filter(
              (item) =>
                !(
                  item.href === '/services/mastopexy-nipple-correction' &&
                  pathname.startsWith('/mastopexy-nipple-correction')
                ),
            )
            .map((item) => (
              <li key={item.href}>
