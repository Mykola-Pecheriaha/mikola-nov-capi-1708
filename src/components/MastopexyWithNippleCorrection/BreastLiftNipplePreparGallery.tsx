...
interface BreastLiftNipplePreparGalleryProps {
  images: GalleryImage[];
}

export function BreastLiftNipplePreparGallery({ images }: BreastLiftNipplePreparGalleryProps) {
      ...
      >
        ...
        <div className="relative w-[calc(100%-8px)] h-[calc(100%-48px)] mx-1 my-1 mb-11 overflow-hidden rounded-lg">
          {!imagesLoaded[currentIndex] && (
            <div
              className="absolute top-0 left-0 w-full h-full flex items-center justify-center text-gray-700 dark:text-gray-300 text-base z-[1]"
              style={{ backgroundColor: 'rgba(142, 170, 184, 0.1)' }}
            >
              Завантаження...
            </div>
          )}
                ...
                {images.map((image, index) => (
                  ...
                  >
                    <Image
                      src={
                        image.src ||
                        '/placeholder.svg?height=75&width=100&query=preparation+thumbnail'
                      }
                      alt={image.alt}
                      width={100}
                      height={75}
...
export default BreastLiftNipplePreparGallery;
