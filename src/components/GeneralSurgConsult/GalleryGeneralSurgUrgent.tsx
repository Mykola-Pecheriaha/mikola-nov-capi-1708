...
export type GalleryGeneralSurgUrgentImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const urgentImages: GalleryGeneralSurgUrgentImage[] = [
  { src: "/images/general-surg-consult/general-surg-consult17.jpg", alt: "абсцеси", width: 800, height: 600, title: "абсцеси" },
  { src: "/images/general-surg-consult/general-surg-consult18.jpg", alt: "Невідкладний візит 2", width: 800, height: 600, title: "біль у животі" },
  { src: "/images/general-surg-consult/general-surg-consult19.jpg", alt: "Невідкладний візит 3", width: 800, height: 600, title: "задишка" },
  { src: "/images/general-surg-consult/general-surg-consult20.jpg", alt: "Невідкладний візит 4", width: 800, height: 600, title: "травми шкіри" },
  { src: "/images/general-surg-consult/general-surg-consult21.jpg", alt: "Невідкладний візит 5", width: 800, height: 600, title: "травми живота" },
  { src: "/images/general-surg-consult/general-surg-consult22.jpg", alt: "Невідкладний візит 6", width: 800, height: 600, title: "мерзлякуватість ніг" },
  { src: "/images/general-surg-consult/general-surg-consult23.jpg", alt: "Невідкладний візит 7", width: 800, height: 600, title: "припухлість, почервоніння " },
  { src: "/images/general-surg-consult/general-surg-consult24.jpg", alt: "Невідкладний візит 8", width: 800, height: 600, title: " вростання нігтя" },
];

interface GalleryGeneralSurgUrgentProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryGeneralSurgUrgent({ className = "", style }: GalleryGeneralSurgUrgentProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(urgentImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % urgentImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + urgentImages.length) % urgentImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!urgentImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = urgentImages[currentIndex];
