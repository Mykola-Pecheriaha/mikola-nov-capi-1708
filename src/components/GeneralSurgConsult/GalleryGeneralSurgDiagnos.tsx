...
export type GalleryGeneralSurgDiagnosImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const diagnosImages: GalleryGeneralSurgDiagnosImage[] = [
  { src: "/images/general-surg-consult/general-surg-consult9.jpg", alt: "Додаткове обстеження 1", width: 800, height: 600, title: "Обстеження 1" },
  { src: "/images/general-surg-consult/general-surg-consult10.jpg", alt: "Додаткове обстеження 2", width: 800, height: 600, title: "Обстеження 2" },

  { src: "/images/general-surg-consult/general-surg-consult11.jpg", alt: "Додаткове обстеження 4", width: 800, height: 600, title: "Обстеження 4" },
  { src: "/images/general-surg-consult/general-surg-consult12.jpg", alt: "Додаткове обстеження 5", width: 800, height: 600, title: "Обстеження 5" },
  { src: "/images/general-surg-consult/general-surg-consult13.jpg", alt: "Додаткове обстеження 6", width: 800, height: 600, title: "Обстеження 6" },
  { src: "/images/general-surg-consult/general-surg-consult14.jpg", alt: "Додаткове обстеження 7", width: 800, height: 600, title: "Обстеження 7" },
  { src: "/images/general-surg-consult/general-surg-consult15.jpg", alt: "Додаткове обстеження 8", width: 800, height: 600, title: "Обстеження 8" },
  { src: "/images/general-surg-consult/general-surg-consult16.jpg", alt: "Додаткове обстеження 9", width: 800, height: 600, title: "Обстеження 9" },
];

interface GalleryGeneralSurgDiagnosProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryGeneralSurgDiagnos({ className = "", style }: GalleryGeneralSurgDiagnosProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(diagnosImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % diagnosImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + diagnosImages.length) % diagnosImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!diagnosImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = diagnosImages[currentIndex];
