...
export type GalleryGeneralSurgConsultImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const generalSurgImages: GalleryGeneralSurgConsultImage[] = [
  { src: "/images/general-surg-consult/general-surg-consult1.jpg", alt: "Консультація хірурга 1", width: 800, height: 600, title: "Консультація 1" },
  { src: "/images/general-surg-consult/general-surg-consult2.jpg", alt: "Консультація хірурга 2", width: 800, height: 600, title: "Консультація 2" },
  { src: "/images/general-surg-consult/general-surg-consult3.jpg", alt: "Консультація хірурга 3", width: 800, height: 600, title: "Консультація 3" },
  { src: "/images/general-surg-consult/general-surg-consult4.jpg", alt: "Консультація хірурга 4", width: 800, height: 600, title: "Консультація 4" },
  { src: "/images/general-surg-consult/general-surg-consult5.jpg", alt: "Консультація хірурга 5", width: 800, height: 600, title: "Консультація 5" },
  { src: "/images/general-surg-consult/general-surg-consult6.jpg", alt: "Консультація хірурга 6", width: 800, height: 600, title: "Консультація 6" },
  { src: "/images/general-surg-consult/general-surg-consult7.jpg", alt: "Консультація хірурга 7", width: 800, height: 600, title: "Консультація 7" },
  { src: "/images/general-surg-consult/general-surg-consult8.jpg", alt: "Консультація хірурга 8", width: 800, height: 600, title: "Консультація 8" },
];

interface GalleryGeneralSurgConsultProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryGeneralSurgConsult({ className = "", style }: GalleryGeneralSurgConsultProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(generalSurgImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % generalSurgImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + generalSurgImages.length) % generalSurgImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!generalSurgImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = generalSurgImages[currentIndex];
