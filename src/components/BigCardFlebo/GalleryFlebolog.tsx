...
export type GalleryFlebologImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї флебології
const flebologImages: GalleryFlebologImage[] = [
  {
    src: "/images/varicose/varicos.jpg",
    alt: "Флебологія 1",
    width: 800,
    height: 600,
...
interface GalleryFlebologProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryFlebolog({ className = "", style }: GalleryFlebologProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(flebologImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % flebologImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + flebologImages.length) % flebologImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!flebologImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = flebologImages[currentIndex]
...
export default GalleryFlebolog
