...
export type GalleryGernioImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї гриж
const gernioImages: GalleryGernioImage[] = [
  {
    src: "/images/gernio/gerniyaDetalis1.jpg",
    alt: "Грижа 1",
    width: 800,
    height: 600,
  ...
  {
    src: "/images/gernio/gerniyaDetalis12.jpg",
    alt: "Грижа 2",
    width: 800,
    height: 600,
...
interface GalleryGernioProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryGernio({ className = "", style }: GalleryGernioProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(gernioImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % gernioImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + gernioImages.length) % gernioImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!gernioImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = gernioImages[currentIndex]
