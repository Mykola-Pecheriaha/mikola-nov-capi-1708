...
export function FaceliftFAQGallery({ images }: FaceliftFAQGalleryProps) {
  ...
  if (!images || images.length === 0) {
    return (
      <div className="w-full h-80 sm:h-96 md:h-[700px] flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-lg text-gray-500 dark:text-gray-400 text-lg sm:text-xl">
  ...
  return (
    <div className="w-full mx-auto relative">
      <div className="relative w-full h-64 sm:h-80 md:h-[500px] lg:h-[700px] bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-xl">
        {/* Навігаційні кнопки */}
        {images.length > 1 && (
          <>
            <button
                ...
                {images.map((image, index) => (
                  ...
                  >
                    <Image
                      src={
                        image.src ||
                        '/placeholder.svg?height=75&width=100&query=facelift faq thumbnail'
                      }
                      alt={image.alt}
                      width={100}
                      height={75}
