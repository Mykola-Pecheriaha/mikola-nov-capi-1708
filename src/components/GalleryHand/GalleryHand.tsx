...
// Масив зображень для галереї хірургії кисті
const handImages: GalleryHandImage[] = [
  {
    src: "/images/hand/carpale17.jpg",
    alt: "Операція кисті 1",
    width: 800,
    height: 600,
    title: "До/Після лікування кисті 1"
  },
  {
    src: "/images/hand/carpale25.jpg",
    alt: "Операція кисті 2",
    width: 800,
    height: 600,
    title: "До/Після лікування кисті 2"
  },
  // Додайте ще зображення за потреби
];

interface GalleryHandProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryHand({ className = "", style }: GalleryHandProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(handImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % handImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + handImages.length) % handImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!handImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = handImages[currentIndex]
    ...
    <div className={`w-full relative ${className}`} style={style}>
      <div className="relative w-full h-[400px] bg-white rounded-lg overflow-hidden shadow-lg">
        ...
        <div className="relative w-[calc(100%-8px)] h-[calc(100%-48px)] m-1 mx-1 mb-11 overflow-hidden rounded">
          {!imagesLoaded[currentIndex] && <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black/10 text-amber-800 text-base z-10">Завантаження...</div>}
        ...
        <button className="absolute top-1/2 -translate-y-1/2 right-4 w-9 h-9 bg-white/90 border-none rounded-full flex items-center justify-center text-lg cursor-pointer transition-all amber-300 z-30 text-amber-500 hover:bg-amber hover:scale-110" onClick={nextImage} aria-label="Наступне зображення">&#10095;</button>
        <div className="absolute bottom-0 left-0 right-0 h-11 bg-white flex items-center justify-between px-4 z-20">
          <div className="flex justify-center items-center gap-2 flex-1">
            {handImages.map((_, idx) => (
