...
export type GalleryСoccyxСystRelapseImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
const relapseImages: GalleryСoccyxСystRelapseImage[] = [
  { src: "/images/procto/coccyxcyst/coccyxcyst18.jpg", alt: "Рецидив куприкового ходу 1", width: 800, height: 600, title: "Рецидив 1" },
  { src: "/images/procto/coccyxcyst/coccyxcyst16.jpg", alt: "Рецидив куприкового ходу 2", width: 800, height: 600, title: "Рецидив 2" },
  { src: "/images/procto/coccyxcyst/coccyxcyst17.jpg", alt: "Рецидив куприкового ходу 3", width: 800, height: 600, title: "Рецидив 3" },

];

interface GalleryСoccyxСystRelapseProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryСoccyxСystRelapse({ className = "", style }: GalleryСoccyxСystRelapseProps) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [fullscreen, setFullscreen] = useState(false);
  const [showThumbnails, setShowThumbnails] = useState(false);
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(relapseImages.length).fill(false));

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % relapseImages.length);
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + relapseImages.length) % relapseImages.length);
  const selectImage = (idx: number) => setCurrentIndex(idx);
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  };

  if (!relapseImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>;
  const currentImage = relapseImages[currentIndex];
