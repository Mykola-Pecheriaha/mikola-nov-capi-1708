...
const menu = [
  ...
  { label: 'Грижесічення з абдомінопластикою ', href: '/services/hernia-abdominoplasty' },
  { label: 'Пластика білої лінії живота', href: '/services/general-white-line' },
];
export default function SidebarGeneraChole() {
        ...
        <ul className="space-y-3 text-base font-medium">
          {menu
            .filter(
              (item) =>
                !(
                  item.href === '/services/general-cholecystectomy' &&
                  pathname.startsWith('/services/general-cholecystectomy')
                ),
            )
            .map((item) => (
              <li key={item.href}>
