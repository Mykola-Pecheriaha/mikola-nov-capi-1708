...
export type GalleryAmbulateImage = {
  src: string;
  alt: string;
  width: number;
  height: number;
...
// Масив зображень для галереї амбулаторної хірургії
const ambulateImages: GalleryAmbulateImage[] = [
  {
    src: "/images/ambulate/resultbreast5.jpg",
    alt: "Амбулаторна хірургія 1",
    width: 800,
    height: 600,
    title: "До/Після лікування 1"
  },
  {
    src: "/images/ambulate/dermatoskop.jpg",
    alt: "Амбулаторна хірургія 2",
    width: 800,
    height: 600,
    title: "До/Після лікування 2"
  },
    {
    src: "/images/ambulate/skintumors5.jpg",
    alt: "Амбулаторна хірургія 2",
    width: 800,
    height: 600,
    title: "До/Після лікування 2"
...
interface GalleryAmbulateProps {
  className?: string;
  style?: React.CSSProperties;
}

export function GalleryAmbulate({ className = "", style }: GalleryAmbulateProps) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [fullscreen, setFullscreen] = useState(false)
  const [showThumbnails, setShowThumbnails] = useState(false)
  const [imagesLoaded, setImagesLoaded] = useState<boolean[]>(() => new Array(ambulateImages.length).fill(false))

  const nextImage = () => setCurrentIndex((prev) => (prev + 1) % ambulateImages.length)
  const prevImage = () => setCurrentIndex((prev) => (prev - 1 + ambulateImages.length) % ambulateImages.length)
  const selectImage = (idx: number) => setCurrentIndex(idx)
  const handleImageLoad = (idx: number) => {
    const arr = [...imagesLoaded]; arr[idx] = true; setImagesLoaded(arr);
  }

  if (!ambulateImages.length) return <div className="w-full h-[400px] flex items-center justify-center bg-gray-100 rounded-lg text-gray-500 text-[1.2rem]">Зображення недоступні</div>
  const currentImage = ambulateImages[currentIndex]
...
export default GalleryAmbulate
